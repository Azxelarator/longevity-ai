...
- เพิ่ม combine.py (GBoost + LLM, logistic 5-fold CV + McNemar); baseline.py บันทึก results/baseline_pairs.csv

## 2026-09-25 (ต่อ) — รวม GBoost + LLM: ไม่ได้อะไรเพิ่ม
- ถูกทั้งคู่ 1574 | GB ถูก LLM ผิด 274 | LLM ถูก GB ผิด 90 | ผิดทั้งคู่ 164 | correlation 0.80
- รวม (logistic 5-fold) 88.1% [86.6–89.4] vs GBoost 87.9% — McNemar p = 0.728 -> ไม่ต่าง
- ข้อสรุป: บนผลเลือด LLM ไม่ได้รู้อะไรที่ GBoost ไม่รู้
- มุมมองใหม่: ทายอายุปฏิทินแม่นไม่ใช่เป้า — สิ่งที่มีค่าคือ "age gap" (อายุที่ทาย − อายุจริง) = อายุชีวภาพ
  คำถามถัดไป: คนที่ age gap สูง ตายเร็วกว่าไหม? (ใช้ข้อมูล nhanes_mortality) -> สะพานไปขั้นที่ 6

## 2026-09-25 (ต่อ) — เตรียม age gap -> mortality
- nhanes_mortality_binary: 4206 คน = คนเดียวกับ age_regression ทั้งหมด; มีอายุ (N-year-old), เพศ, สถานะตาย/มีชีวิต + เวลาติดตาม (censored)
- เพิ่ม notebooks/mortality.py: GBoost OOF -> age gap (ปรับด้วยอายุ) -> Cox PH (age + sex + gap), C-index, อัตราตาย 10 ปีตาม tertile

## 2026-09-25 (ต่อ) — GBoost age gap -> mortality: จริงแต่อ่อน
- 4206 คน, ตาย 1348, ติดตามเฉลี่ย 16.9 ปี | อายุสองชุดตรงกัน 100%
- Cox (อายุ+เพศ+gap): gap ทุก 5 ปี HR 1.07 [1.03–1.10], p=0.0002 | อายุ 10 ปี HR 2.42
- C-index 0.845 -> 0.846 (แทบไม่เพิ่ม)
- ตาราง tertile ปนอายุ (กลุ่มแก่อายุเฉลี่ย 53 vs 49) -> ใช้ Cox เป็นหลัก
- เหตุผล: สูตรที่เทรนให้ทายอายุปฏิทิน = first-gen clock, สัญญาณ mortality อ่อน (วงการรู้แล้ว -> PhenoAge)
- เพิ่ม notebooks/phenoage.py: PhenoAge (Levine 2018) vs GBoost บนคนเดียวกัน + ตาราง tertile แบบคุมช่วงอายุ

## 2026-09-25 (ต่อ) — PhenoAge ชนะขาด
- PhenoAge คำนวณได้ 3973/4206 คน | corr กับอายุจริง 0.95 | เฉลี่ยต่ำกว่าอายุจริง 6.4 ปี
- Cox (3973 คน, ตาย 1218): ฐาน อายุ+เพศ C 0.850
  + GBoost gap: HR/5ปี 1.05 [1.02–1.09], C 0.851
  + PhenoAge gap: HR/5ปี 1.33 [1.28–1.39], p=6e-43, C 0.859
  + ทั้งคู่: GBoost 0.98 (p=0.3, ไม่เพิ่มอะไร) | PhenoAge 1.34
- อัตราตาย 10 ปี (คุมช่วงอายุ, tertile PhenoAge gap): 40-60: 4.9 / 3.9 / 11.9% | 60+: 27.6 / 30.9 / 48.7%
- ข้อจำกัด: PhenoAge พัฒนาจาก NHANES III — อาจเป็นคนกลุ่มเดียวกับข้อมูลนี้ (in-sample advantage)
- ข้อสรุปของวัน: ตาชั่งที่ดีที่สุดที่ทดสอบ = PhenoAge (สูตรเปิด 9 ค่าเลือด) > GBoost ของเรา > LLM
